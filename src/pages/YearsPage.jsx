...
const YearsPage = () => {
  const { years, loading, fetchFilieres } = useStore();
          ...
          {loading ? <LoadingSpinner /> : (
            <div className="grid-container grid-3">
              {years.map((year, index) => (
                <div key={year.id} className="card card-hover" onClick={() => fetchFilieres(year)}>
                  <div className="card-header">
                    <div className={`card-icon card-icon-${['green', 'blue', 'orange'][index % 3]}`}>{index + 1}</div>
